...
export async function handleContextAnalysis(request, env) {
  ...
  try {
    const { path, title, content, metadata } = await request.json();
    ...
    const analysis = await performContextAnalysis({ path, title, content, metadata }, env);
...
async function performContextAnalysis(context, env) {
  const { path, title, content, metadata } = context;
...
function detectDocumentType(title, path, content) {
  ...
  return { documentType, contentTheme, visualNeeds, priority, confidence };
}
...
function getRecommendationReason(asset, analysis) {
  ...
  } else if (visualNeeds.includes('hero') && (name.includes('hero') || name.includes('background'))) {
    return 'High-impact visual';
  } else if (asset.usageCount > 2) {
    return 'Popular choice';
  } else if (asset.qualityScore > 80) {
