...
function calculateOverallStatus(services, healthChecks) {
  ...
  const criticalUnavailable = criticalServices.some((service) => services[service] === 'unavailable');

  const healthChecksFailed = Object.values(healthChecks).some((check) => check.status === 'unhealthy');
